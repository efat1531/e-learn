...
const LectureList = ({ lesson }) => {
  ...
  return (
    <div className="flex w-full justify-between p-0 px-6 group hover:cursor-pointer">
      <div
        className={`flex flex-row items-center gap-2 pl-4 ${content._id == lectureId ? "text-Primary-500" : "text-gray-600"
          }`}
      >
      ...
      {content.contentType === "video" && (
        <div
          className={`flex gap-1 items-center  ${content._id == lectureId ? "text-Primary-500" : "text-gray-600"
            }`}
        >
